...
import { ChevronRight, ChevronDown, Globe, MessageCircle, User, Bell, Shield, LogOut, TrendingUp, TrendingDown, Heart, Newspaper, Edit3, Trash2, X } from "lucide-react";
import { ASSETS } from "../data";
import { APP_ASSETS } from "../data";
import { useApp } from "../context/AppContext";

const NotifToggle = ({ label, desc, defaultOn }) => {
        ...
        {!profilePage ? (
            ...
            <div className="grid grid-cols-3 gap-3 mb-8">
              {[
                ...
                <div key={i} className="bg-black/20 border border-white/[0.03] rounded-2xl p-4 text-center">
                  <div className={`text-[22px] font-black bg-gradient-to-r ${s.color} bg-clip-text text-transparent`}>{s.value}</div>
                  <div className="text-[9px] font-bold text-[#7A7B8D] uppercase tracking-widest mt-1">{s.label}</div>
                </div>
              ))}
            </div>
            ...
            <div onClick={() => setProfilePage("comments")} className="bg-gradient-to-r from-[#B24BF3]/10 to-[#5B7FFF]/10 border border-[#B24BF3]/20 rounded-[24px] p-5 mb-4 cursor-pointer hover:from-[#B24BF3]/15 hover:to-[#5B7FFF]/15 transition-all">
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-4">
                  <div className="w-11 h-11 rounded-xl bg-gradient-to-br from-[#B24BF3] to-[#5B7FFF] flex items-center justify-center shadow-[0_0_15px_rgba(178,75,243,0.3)]"><MessageCircle className="w-5 h-5 text-black" /></div>
                  <div>
          ...
          <motion.div key="profile-account" initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }}>
            ...
            <div className="flex items-center justify-between mb-6">
              ...
              <button onClick={() => setIsEditingProfile(e => !e)} className={`px-4 py-2 rounded-xl text-[10px] font-black uppercase tracking-widest transition-all ${isEditingProfile ? "bg-gradient-to-r from-[#00FFFF] to-[#39FF14] text-black" : "bg-white/5 text-white/50 border border-white/10"}`}>
            ...
            <div className="space-y-4">
              <div className="bg-black/20 border border-white/[0.03] rounded-2xl p-4">
                ...
                {isEditingProfile
                  ? <input value={editedUsername} onChange={e => setEditedUsername(e.target.value)} className="w-full bg-white/5 border border-[#00FFFF]/30 rounded-xl px-3 py-2 text-[15px] font-bold text-white focus:outline-none" />
              ...
              {[
                ...
                <div key={i} className="bg-black/20 border border-white/[0.03] rounded-2xl p-4">
                  <div className="text-[9px] font-bold text-[#7A7B8D] uppercase tracking-widest mb-1">{f.label}</div>
                  <div className="text-[15px] font-bold text-white">{f.value}</div>
                </div>
              ))}
            </div>
          </motion.div>
          ...
          <motion.div key="profile-notif" initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }}>
            ...
            <div className="space-y-3">
              ...
              ].map((item, i) => <NotifToggle key={i} {...item} />)}
            </div>
          </motion.div>
