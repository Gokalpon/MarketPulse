...
export function Watchlist() {
  const { watchlistAssets, watchlistLayout, setWatchlistLayout, navigateTo, t } = useApp();
    ...
    <motion.div key="watchlist" initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }} className="px-6 pt-12 pb-24">
      ...
      {watchlistLayout === "list" ? (
        <div className="flex flex-col gap-3">
          {assets.map(asset => (
            <div key={asset.id} onClick={() => navigateTo(asset.id)} className="bg-black/20 border border-white/[0.03] rounded-2xl p-4 flex items-center justify-between hover:bg-black/30 transition-colors cursor-pointer">
        ...
        <div className="grid grid-cols-2 gap-4">
          {assets.map(asset => (
            <div key={asset.id} onClick={() => navigateTo(asset.id)} className="bg-black/20 border border-white/[0.03] rounded-[24px] p-5 flex flex-col hover:bg-black/30 transition-colors cursor-pointer">
              ...
              <div className="mt-auto">
                ...
                <div className="flex items-center justify-between">
                  <Sparkline data={asset.data["1D"].slice(-20)} color={asset.isUp ? "#39FF14" : "#E50000"} />
                  <div className={`text-[9px] font-bold px-1 py-0.5 rounded ${asset.change.startsWith("+") ? "bg-gradient-to-r from-[#00FFFF] to-[#39FF14] text-black" : asset.change.startsWith("-") ? "bg-[#E50000] text-black" : "bg-white/10 text-white"}`}>{asset.change}</div>
                </div>
              </div>
            </div>
          ))}
