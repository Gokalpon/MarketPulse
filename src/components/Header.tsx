          ...
          <>
            ...
            <motion.div initial={{ x: "-100%" }} animate={{ x: 0 }} exit={{ x: "-100%" }} transition={{ type: "tween", duration: 0.25, ease: "easeOut" }} className="absolute top-0 left-0 bottom-0 w-[300px] bg-black/80 backdrop-blur-xl border-r border-white/[0.05] z-[160] p-6 flex flex-col">
              ...
              <div className="flex items-center justify-between px-2 mt-4 mb-2">
                ...
                <button onClick={e => { e.stopPropagation(); setIsEditPinned(!isEditPinned); }} className={`px-3 py-1 rounded-lg text-[8px] font-black uppercase tracking-[0.2em] transition-all border ${isEditPinned ? "bg-white text-black border-white" : "bg-white/5 text-white border-white/10"}`}>
              ...
              <div className="flex flex-col gap-2 overflow-y-auto flex-1 pr-2 scrollbar-hide">
                {isEditPinned ? (
                  <>
                    ...
                    <div className="px-2 mb-4 mt-6">
                      ...
                      <div className="relative">
                        ...
                        <input type="text" placeholder={t.searchToAdd} value={menuSearch} onChange={e => setMenuSearch(e.target.value)} className="w-full bg-white/5 border border-white/10 rounded-xl py-2.5 pl-9 pr-4 text-[12px] focus:outline-none focus:border-[#00FFFF]/50 transition-colors" />
                      </div>
                    </div>
                    ...
                    <div className="flex flex-col gap-2">
                      {menuSearch.length > 0 && ASSETS.filter(a => !pinnedAssets.includes(a.id) && (a.id.toLowerCase().includes(menuSearch.toLowerCase()) || a.name.toLowerCase().includes(menuSearch.toLowerCase()))).slice(0, 10).map(asset => (
                        <div key={asset.id} className="flex items-center justify-between px-4 py-3 rounded-2xl bg-white/5 hover:bg-white/[0.07] transition-colors">
                          ...
                          <button onClick={() => { setPinnedAssets([...pinnedAssets, asset.id]); setMenuSearch(""); }} className="w-6 h-6 rounded-full flex items-center justify-center bg-gradient-to-br from-[#00FFFF] to-[#39FF14] text-black shadow-sm hover:scale-110 transition-transform">
                  ...
                  pinnedAssets.map(id => {
                    ...
                    return (
                      <div key={id} onClick={() => navigateTo(id)} className="flex items-center justify-between px-4 py-3 rounded-2xl hover:bg-white/5 cursor-pointer transition-colors">
                        <div className="flex items-center gap-3">
                          <div className="w-8 h-8 rounded-lg bg-white/5 flex items-center justify-center text-xs font-bold border border-white/[0.05]">{asset.id[0]}</div>
                          <div>
                            <div className="font-bold text-[14px]">{asset.id}</div>
                        ...
                        <div className="text-right">
                          ...
                          <div className={`text-[9px] font-bold ${asset.change.startsWith("+") ? "text-[#39FF14]" : "text-[#E50000]"}`}>{asset.change}</div>
                        </div>
                      </div>
                    );
                  })
