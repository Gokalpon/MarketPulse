...
export default function App() {
  ...
  useEffect(() => {
    // Sync with Supabase user if present, but don't force logout if manual bypass is used
    if (user) {
      setIsLoggedIn(true);
  ...
  const fetchRealComments = async () => {
    ...
    try {
      ...
      if (!error && data) {
        // Map postgres comments to app state comments format
        const realComments = data.map(dbComment => ({
           id: dbComment.id,
           assetId: dbComment.asset_id,
           timeframe: dbComment.timeframe,
           chartIndex: 0, // lightweight chart won't care about CSS index
           price: dbComment.price,
           text: dbComment.text,
           sentiment: dbComment.sentiment,
           timestamp: dbComment.created_at,
           user: `@${dbComment.users?.username || 'User'}`,
           likes: dbComment.likes,
           realTime: new Date(dbComment.created_at).getTime() / 1000 // UNIX seconds
        }));
        setUserComments(realComments);
      }
  ...
  const loadRealData = async () => {
    ...
    console.log(`Fetching real data for ${selectedAssetId}...`);
    try {
      ...
      if (data && data.length > 2) { // Need at least 2 points to draw a line
        ...
        console.log(`Failed or insufficient data (${data?.length || 0} points). Falling back to mock.`);
      ...
      if (quote) {
        ...
        console.warn("Real-time quote failed, using price fetch.");
        const price = await fetchRealTimePrice(selectedAssetId);
        if (price) setRealTimePrice(price);
      }
    } catch (err) {
      console.error("LoadRealData failed:", err);
      setRealMarketData(null);
    }
  ...
  useEffect(() => {
    loadRealData();
    // Set up a refresh every 3 minutes for current price (stay within free tier limits)
    const interval = setInterval(() => {
      fetchQuote(selectedAssetId).then(q => {
        if (q) {
          setRealQuote(q);
  ...
  // Generate strict chronologic timestamps for Lightweight Charts (UNIX seconds)
  const chartDataPoints = useMemo(() => {
    ...
    return activeData.map((val, i) => ({
      time: (now - (activeData.length - 1 - i) * intervalSeconds) as any,
      value: val,
      originalIndex: i
    }));
  }, [activeData, timeframe, realMarketData]);

  // Map our UI comments and translations to ChartComponent markers
  const chartMarkers = useMemo(() => {
    ...
    if (showAIConsensus || showNewsBubbles) {
      activeTranslations.forEach((point: any) => {
        ...
        const targetPoint = chartDataPoints[point.idx] || chartDataPoints[0];
        if (targetPoint) {
          markers.push({
             time: targetPoint.time,
             sentiment: point.type === "news" ? 'Neutral' : point.sentiment, 
             type: point.type,
             idx: point.idx
          });
        }
    ...
    // Also add user comments as markers
    activeUserComments.forEach(uc => {
       const timeVal = uc.realTime ? uc.realTime : (chartDataPoints[uc.chartIndex]?.time || chartDataPoints[chartDataPoints.length-1].time);
       if (timeVal) {
         markers.push({
            time: timeVal,
                    ...
                    <div>
                      ...
                      <div className="flex items-center gap-3">
                        <div className={`px-2 py-1 rounded-lg flex items-center gap-1 font-bold text-[11px] transition-all ${
                          ...
                          {(realQuote ? realQuote.isUp : activeAsset.change.startsWith('+')) ? <TrendingUp className="w-3 h-3" strokeWidth={3} /> : <TrendingDown className="w-3 h-3" strokeWidth={3} />}
                  ...
                  <div className="mt-8 relative h-[240px] w-full rounded-2xl overflow-hidden border border-white/5 shadow-[0_0_30px_rgba(0,0,0,0.5)]">
                    <MarketPulseChart 
                      ...
                      onCrosshairMove={(param: any) => {
                         ...
                         // For Lightweight Charts, crosshair move gives us price and time
                         let price = 0;
                         param.seriesData.forEach((val: any) => { price = val.value || price; });
                         setChartCrosshair({ 
                            idx: 0, // Using time directly instead
                            price, 
                            x: 50, // Ignored by new chart since crosshair is internal
                            y: 50 
                         });
                      }}
                    />
              ...
              <AnimatePresence mode="wait">
                {!profilePage ? (
                  ...
                  <motion.div key="profile-account" initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }}>
                    <button onClick={() => { setProfilePage(null); setIsEditingProfile(false); }} className="flex items-center gap-2 text-[#7A7B8D] text-[12px] font-bold uppercase tracking-wider mb-6 hover:text-white transition-colors"><ChevronRight className="w-4 h-4 rotate-180" /> Profile</button>
                    <div className="flex items-center justify-between mb-6">
                      <h3 className="text-xl font-black uppercase">{t.accountSettings}</h3>
                      <button
                        onClick={() => setIsEditingProfile(e => !e)}
                        className={`px-4 py-2 rounded-xl text-[10px] font-black uppercase tracking-widest transition-all ${
                          isEditingProfile ? 'bg-gradient-to-r from-[#00FFFF] to-[#39FF14] text-black' : 'bg-white/5 text-white/50 border border-white/10'
                        }`}
                      >
                        {isEditingProfile ? (language === "Turkish" ? "Kaydet" : "Save") : (language === "Turkish" ? "Düzenle" : "Edit")}
                      </button>
                    </div>
                    <div className="space-y-4">
                      <div className="bg-black/20 border border-white/[0.03] rounded-2xl p-4">
                        <div className="text-[9px] font-bold text-[#7A7B8D] uppercase tracking-widest mb-1">{language === "Turkish" ? "Kullanıcı Adı" : "Username"}</div>
                        {isEditingProfile ? (
                          <input
                            value={editedUsername}
                            onChange={(e) => setEditedUsername(e.target.value)}
                            className="w-full bg-white/5 border border-[#00FFFF]/30 rounded-xl px-3 py-2 text-[15px] font-bold text-white focus:outline-none focus:border-[#00FFFF]/60 transition-colors"
                          />
                        ) : (
                          <div className="text-[15px] font-bold text-white">{editedUsername}</div>
                        )}
                      </div>
                      {[
            ...
            >
              ...
              <div className="flex items-center justify-between mb-4">
                <div className="text-[13px] font-bold text-white/90">
                  {detailedPoint.comments?.length || 0} {language === "Turkish" ? "Yorum" : "Comments"}
                </div>
                <div className="flex items-center gap-3">
                  <span className="flex items-center gap-1 text-[11px] font-black text-[#39FF14]">
                    <TrendingUp className="w-3.5 h-3.5" strokeWidth={3} />
                    {detailedPoint.comments?.filter((c: any) => c.sentiment === "Positive").length || 0}
                  </span>
                  <span className="flex items-center gap-1 text-[11px] font-black text-[#FF3131]">
                    <TrendingDown className="w-3.5 h-3.5" strokeWidth={3} />
                    {detailedPoint.comments?.filter((c: any) => c.sentiment === "Negative").length || 0}
                  </span>
                </div>
              </div>
                    ...
                    <div className="mt-3 flex items-center justify-between">
                      <div className="flex items-center gap-2 text-[10px] font-bold">
                        <button 
                          ...
                          className={`flex items-center gap-1 transition-all ${
                            commentVotes[`${detailedPoint?.idx}-${i}`] === 'up' 
                              ? 'text-[#39FF14]' 
                              : 'text-[#7A7B8D] hover:text-white/60'
                          }`}
                        >
                        ...
                        <button 
                          ...
                          className={`flex items-center gap-1 transition-all ml-2 ${
                        ...
                        <button
                          onClick={() => setCommentText(`@${c.user.replace('@','')} `)}
                          className="flex items-center gap-1 text-[#7A7B8D] hover:text-[#00FFFF] transition-colors ml-3"
                        >
                          <MessageCircle className="w-3.5 h-3.5" strokeWidth={2.5} />
                          <span className="opacity-0 group-hover:opacity-100 hidden sm:inline-block transition-opacity">Reply</span>
                        </button>
                      </div>
                      {autoTranslate && (
                        <div className="flex items-center gap-1 opacity-40">
                          <Globe className="w-2.5 h-2.5 text-white" />
                          <span className="text-[8px] text-white tracking-widest uppercase">{language}</span>
                        </div>
                      )}
                    </div>
                  </div>
